 ...
 *
 * <p>Given a list of accounts where each element accounts[i] is a list of strings, where the first
 ...
 * Explanation: The first and third John's are the same person as they have the common email
 ...
 * 'john_newyork@mail.com', 'johnsmith@mail.com']] would still be accepted. Example 2:
 *
 * <p>Input: accounts =
 ...
 * <p>Constraints:
 *
 * <p>1 <= accounts.length <= 1000 2 <= accounts[i].length <= 10 1 <= accounts[i][j] <= 30
 * accounts[i][0] consists of English letters. accounts[i][j] (for j > 0) is a valid email.
 */
public class AccountsMerge {
  ...
  public List<List<String>> accountsMerge(List<List<String>> accounts) {
    ...
    Map<String, Account> emailToAccountMap = new HashMap<>();
    for (List<String> account : accounts) {
      ...
      for (int i = 1; i < account.size(); i++) {
        ...
        Account mappedAcct = emailToAccountMap.get(mail);
        if (mappedAcct == null) {
          emailToAccountMap.put(mail, acct);
          acct.mails.add(mail);
        } else {
